...
describe("SkeuomorphicKit shell", () => {
  ...
  it("provides a functional calendar month and agenda view", async () => {
    ...
    // The time picker restores focus to its trigger inside a requestAnimationFrame;
    // wait for that to land before typing so the deferred restore cannot race the
    // typing focus. The lazy catalog never mounts in tests (see src/test/setup.ts),
    // so its resizable panels' document-level pointerdown focus steal cannot occur.
    await waitFor(() =>
